...
const getEmails = async () => {
    let emails = path.join(__dirname, '..', '..', '.github', '.maillist');
    ...
    try {
        ...
        for (const email of emails) {
            const info = {
                ...
                html: Template.replace('{{error.message}}', message)
                    .replace('{{errorType}}', errorType)
                    .replace('{{error.stack}}', stack),
